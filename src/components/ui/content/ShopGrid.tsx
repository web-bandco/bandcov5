...
import { 
  X, ChevronDown, ChevronUp, ArrowDownAZ, ArrowUpZA, 
  TrendingUp, TrendingDown, Sparkles, History, Filter, 
  SlidersHorizontal, RefreshCw, PackageX 
} from "lucide-react" 
...
export interface ProductImage { thumb: string; full: string; }
export interface Product {
  id: string; title: string; price: string; description: string;
  images: ProductImage[]; url: string; condition?: string; size?: string;
...
function ProductCard({ product, storeName, gradientStyle, isPriority }: { product: Product, storeName: string, gradientStyle: React.CSSProperties, isPriority?: boolean }) {
  ...
  React.useEffect(() => {
    const checkTruncation = () => {
      if (titleRef.current && !isTitleExpanded) {
        setShowTitleToggle(titleRef.current.scrollHeight > titleRef.current.clientHeight + 2);
      }
    };
    checkTruncation();
  ...
  React.useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowLeft") { e.preventDefault(); e.stopPropagation(); dialogApi.scrollPrev(); } 
      else if (e.key === "ArrowRight") { e.preventDefault(); e.stopPropagation(); dialogApi.scrollNext(); }
    };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <div className="card-body p-4 md:p-5 flex flex-col flex-grow">
          <div className="flex items-start justify-between gap-3 mb-2 min-h-[3.5rem]">
            <div className="relative flex-1 min-w-0">
              ...
              {showTitleToggle && (
                <button
                  onClick={(e) => { e.stopPropagation(); setIsTitleExpanded(!isTitleExpanded); }}
                ...
                >
                  {isTitleExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                </button>
              )}
            </div>
      ...
      {/* LIGHTBOX */}
      {isMounted && (
        <DialogContent showCloseButton={false} className="max-w-none sm:max-w-none w-screen h-[100dvh] bg-transparent border-none shadow-none p-0 flex flex-col justify-center overflow-hidden data-[state=open]:animate-none outline-none">
          <DialogTitle className="sr-only">{product.title} Lightbox View</DialogTitle>
          <button onClick={() => setIsOpen(false)} className="absolute top-4 right-4 md:top-6 md:right-6 z-[100] p-3 rounded-full bg-background/60 hover:bg-background backdrop-blur-md transition-all text-foreground outline-none shadow-md cursor-pointer"><X className="w-6 h-6 md:w-8 md:h-8" /></button>
          <button onClick={() => dialogApi?.scrollPrev()} className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 z-[100] hidden md:flex w-14 h-14 bg-background/60 hover:bg-background border-none shadow-md backdrop-blur-md rounded-full items-center justify-center text-foreground outline-none transition-all cursor-pointer"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-7 h-7"><path d="m15 18-6-6 6-6"/></svg></button>
...
export function ShopGrid({ storeName, children }: ShopGridProps) {
    ...
    const fetchLiveProducts = async () => {
      try {
        ...
        const formattedData = data.map((item: any) => ({
          ...item, id: item._id, createdAt: item._createdAt, category: item.category, size: item.size, condition: item.condition,
          images: item.images?.map((img: any) => ({
            thumb: urlFor(img).width(600).format('webp').quality(80).url(), full: urlFor(img).width(1600).format('webp').quality(90).url()
          })) || []
        }));
  ...
  const availableCategories = React.useMemo(() => Array.from(new Set(products.map(p => p.category).filter(Boolean) as string[])).sort(), [products]);
  const availableSizes = React.useMemo(() => Array.from(new Set(products.map(p => p.size).filter(Boolean) as string[])).sort(), [products]);
  const availableConditions = React.useMemo(() => Array.from(new Set(products.map(p => p.condition).filter(Boolean) as string[])).sort(), [products]);
  ...
  const getCategoryCount = React.useCallback((cat: string) => products.filter(p => p.category === cat && matchesSizes(p) && matchesConditions(p)).length, [products, matchesSizes, matchesConditions]);
  const getSizeCount = React.useCallback((size: string) => products.filter(p => p.size === size && matchesCategories(p) && matchesConditions(p)).length, [products, matchesCategories, matchesConditions]);
  const getConditionCount = React.useCallback((cond: string) => products.filter(p => p.condition === cond && matchesCategories(p) && matchesSizes(p)).length, [products, matchesCategories, matchesSizes]);

  const totalActiveFilters = selectedCategories.length + selectedSizes.length + selectedConditions.length;

  const clearAllFilters = () => {
    setSelectedCategories([]); setSelectedSizes([]); setSelectedConditions([]);
  ...
  const filteredAndSortedProducts = React.useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      const priceA = parsePrice(a.price); const priceB = parsePrice(b.price);
      const dateA = new Date(a.createdAt || 0).getTime(); const dateB = new Date(b.createdAt || 0).getTime();
      ...
      <label className={`flex items-center justify-between cursor-pointer group ${isDisabled ? 'opacity-40 pointer-events-none' : ''}`}>
        <div className="flex items-center gap-3">
          <div className="relative flex items-center justify-center w-5 h-5 border border-border rounded shadow-sm group-hover:border-brand-500 transition-colors bg-background shrink-0">
            <input type="checkbox" className="peer sr-only" checked={isChecked} disabled={isDisabled}
              onChange={(e) => {
                const checked = e.target.checked; onChange(checked);
                trackEvent('use_filter', { filter_type: filterType, filter_value: label, filter_action: checked ? 'add' : 'remove', store_platform: storeName });
              }}
            />
          ...
          <span className="text-sm font-medium text-foreground group-hover:text-brand-500 transition-colors">{label}</span>
        </div>
        <span className="text-xs text-foreground-muted group-hover:text-foreground transition-colors font-medium">({count})</span>
      </label>
    );
  };

  const sortOptionsData = [
    { value: "name-asc", label: "Name: A to Z", icon: ArrowDownAZ }, { value: "name-desc", label: "Name: Z to A", icon: ArrowUpZA },
    { value: "price-asc", label: "Price: Low to High", icon: TrendingUp }, { value: "price-desc", label: "Price: High to Low", icon: TrendingDown },
    { value: "date-desc", label: "Newest First", icon: Sparkles }, { value: "date-asc", label: "Oldest First", icon: History },
  ];
    ...
    return (
      <div className="w-full flex flex-col">
        ...
        <div className="mx-4 md:mx-0 py-16 px-6 bg-surface-primary border border-border/50 rounded-3xl flex flex-col items-center justify-center text-center shadow-sm">
          <div className="w-16 h-16 rounded-2xl bg-brand-500/10 flex items-center justify-center mb-6"><PackageX className="w-8 h-8 text-brand-500" /></div>
          <h3 className="text-xl md:text-2xl font-bold text-foreground mb-3 tracking-tight">There's nothing left...</h3>
          <p className="text-foreground-muted max-w-lg mx-auto text-sm md:text-base leading-relaxed">There are currently no items for sale on the SHOP: B&Co {storeName} store — check back again soon.</p>
        </div>
      </div>
    );
  }
          ...
          <Sheet>
            <SheetTrigger asChild>
              <button className="md:hidden flex-1 flex items-center justify-center gap-2 h-11 px-4 rounded-xl bg-surface-primary border border-border text-foreground shadow-sm hover:shadow-md transition-all font-medium whitespace-nowrap outline-none focus-visible:ring-2 focus-visible:ring-ring">
                <SlidersHorizontal className="w-4 h-4 text-foreground-muted" /><span>Sort & Filter</span>
                {totalActiveFilters > 0 && (<span className="flex items-center justify-center w-5 h-5 rounded-full bg-brand-500 text-white text-[10px] font-bold">{totalActiveFilters}</span>)}
              </button>
            </SheetTrigger>
            <SheetTrigger asChild>
              <button className="hidden md:flex items-center justify-center gap-2 h-11 px-4 rounded-xl bg-surface-primary border border-border text-foreground shadow-sm hover:shadow-md transition-all font-medium whitespace-nowrap outline-none focus-visible:ring-2 focus-visible:ring-ring">
                <Filter className="w-4 h-4 text-foreground-muted" /><span>Filter</span>
                {totalActiveFilters > 0 && (<span className="flex items-center justify-center w-5 h-5 rounded-full bg-brand-500 text-white text-[10px] font-bold">{totalActiveFilters}</span>)}
              </button>
            </SheetTrigger>
            ...
            <SheetContent side="right" className="w-full sm:max-w-md bg-background border-l border-border p-0 flex flex-col z-[150]">
              <SheetHeader className="p-6 border-b border-border text-left">
                <div className="mb-1"><span className="inline-flex items-center justify-center px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-widest bg-brand-500/10 text-brand-700 dark:text-brand-400">SHOP: B&CO - {storeName.toUpperCase()}</span></div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                <div className="md:hidden">
                  <FilterSection title="Sort By">
                    <Select value={sortOption} onValueChange={(val) => { setSortOption(val); trackEvent('sort_inventory', { sort_value: val, store_platform: storeName }); }}>
                      <SelectTrigger aria-label="Sort products" className="w-full bg-surface-primary border-border text-foreground !h-11 rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-ring"><SelectValue placeholder="Newest First" /></SelectTrigger>
                        ...
                        <SelectGroup>
                          {sortOptionsData.map((opt) => (
                            <SelectItem key={opt.value} value={opt.value} className="cursor-pointer py-2 focus:bg-accent focus:text-accent-foreground">
                              <div className="flex items-center gap-2.5"><opt.icon className="w-4 h-4 text-foreground-muted" /><span>{opt.label}</span></div>
                            </SelectItem>
                          ))}
                        </SelectGroup>
                      </SelectContent>
                ...
                {availableCategories.length > 0 && (
                  <FilterSection title="Category">
                    {availableCategories.map(cat => <FilterCheckbox key={cat} label={cat} filterType="Category" count={getCategoryCount(cat)} isChecked={selectedCategories.includes(cat)} onChange={(c) => { if (c) setSelectedCategories(prev => [...prev, cat]); else setSelectedCategories(prev => prev.filter(item => item !== cat)); }} />)}
                  </FilterSection>
                )}
                {availableSizes.length > 0 && (
                  <FilterSection title="Size">
                    {availableSizes.map(size => <FilterCheckbox key={size} label={size} filterType="Size" count={getSizeCount(size)} isChecked={selectedSizes.includes(size)} onChange={(c) => { if (c) setSelectedSizes(prev => [...prev, size]); else setSelectedSizes(prev => prev.filter(item => item !== size)); }} />)}
                  </FilterSection>
                )}
                {availableConditions.length > 0 && (
                  <FilterSection title="Condition">
                    {availableConditions.map(cond => <FilterCheckbox key={cond} label={cond} filterType="Condition" count={getConditionCount(cond)} isChecked={selectedConditions.includes(cond)} onChange={(c) => { if (c) setSelectedConditions(prev => [...prev, cond]); else setSelectedConditions(prev => prev.filter(item => item !== cond)); }} />)}
                  </FilterSection>
                )}
                {totalActiveFilters === 0 && <p className="text-sm text-foreground-muted hidden md:block pt-2">No active filters applied.</p>}
              </div>

              <SheetFooter className="p-6 border-t border-border mt-auto">
                <button onClick={clearAllFilters} disabled={totalActiveFilters === 0} className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-medium text-foreground bg-surface-secondary border border-border transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-accent hover:text-accent-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring">
          ...
          <div className="hidden md:block relative shrink-0 w-[240px]">
            <Select value={sortOption} onValueChange={(val) => { setSortOption(val); trackEvent('sort_inventory', { sort_value: val, store_platform: storeName }); }}>
              <SelectTrigger aria-label="Sort products" className="w-full !h-11 px-4 bg-surface-primary border-border text-foreground rounded-xl shadow-sm hover:shadow-md transition-all font-medium outline-none focus-visible:ring-2 focus-visible:ring-ring">
                <div className="flex items-center gap-2 overflow-hidden"><span className="text-foreground-muted font-normal shrink-0">Sort by:</span><SelectValue placeholder="Newest First" /></div>
              </SelectTrigger>
              <SelectContent position="popper" sideOffset={8} className="rounded-xl border-border bg-background shadow-2xl z-[100]">
                <SelectGroup>
                  {sortOptionsData.map((opt) => (
                    <SelectItem key={opt.value} value={opt.value} className="cursor-pointer py-2 focus:bg-accent focus:text-accent-foreground">
                      <div className="flex items-center gap-2.5"><opt.icon className="w-4 h-4 text-foreground-muted" /><span>{opt.label}</span></div>
